...
% Indirect gradients via btildprime, l1 and l2
   ...
   % FOC with respect to x'(1)
    ...
    grad2(1) = 0; % c_1_1
    grad2(2) = P(s_,2)*(alpha(1)*psi*c1_2^(-1)+beta*c2_2^(-sigma)*dV(2));  % c_1_2
    grad2(3) = 0; % c_2_1
    ...
    % FOC with x'(2)
    res(5) = P(s_,2)*lambda_I(2)+P(s_,2)*beta*dV(1)+MuL(2)-MuH(2); % Anmol - Changed the sign on MuH to minus
    % Definition of x'(2)
