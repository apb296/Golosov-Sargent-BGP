...
alpha1GridSize=20;
alpha1grid=linspace(0.65, 0.8,alpha1GridSize);
tau1Target=.2;
c10guess=1;
c20guess=1/(RMax);
RGrid=[];
...
Para.alpha_2=alpha_2;
   ...
   [res]=GetSteadyStateMoments(Para,c10guess,c20guess);
     ...
     l1(i)=res.l1;
     l2(i)=res.l2;
     YSim(i) = theta_1*l1(i)*Para.n1+theta_2*l2(i)*Para.n2;
     ToverY(i) = (Para.n1+Para.n2)*res.Trans/YSim(i);
     goverY(i) = g/YSim(i);
     btild(i)=res.btild;
     resid(i) = tau1(i)*YSim(i)-(Para.n1+Para.n2)*res.Trans-Para.g(1);
end
...
ylabel('btild')
figure()
plot(alpha1grid,resid,'k','LineWidth',2);
xlabel('alpha_1')
ylabel('government residual')
